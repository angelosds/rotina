...
5. Publicar staging e validar login real, convites, mobile/desktop, expiração, revogação e permissões. A publicação, a entrega e o consumo de links, a sessão do proprietário, a proteção anônima, os layouts mobile, o aceite, o primeiro acesso, a revogação de convite pendente e a separação dos papéis `owner` e `member` foram validados no ambiente hospedado. A conta convidada recebeu página não encontrada ao acessar diretamente a administração. Em 20/09/2026, a administração de membros foi publicada e validada no staging em desktop e celular: listagem, estados ativo/suspenso, confirmação antes da suspensão, reativação, encerramento seletivo de sessões e bloqueio de novos links de acesso, preservando conta e dados. Em 21/09/2026, cartões, faturas, compras pontuais e parceladas foram validados em staging: interpretação em linguagem natural, atribuição pelo fechamento, mudança completa dos lançamentos entre meses, parcelas mensais, projeto, tags e pagamento parcial sem duplicar despesas. O layout mobile em modo escuro também foi verificado sem erros no console.
6. Promover o mesmo commit validado para main, executar migrações compatíveis em produção e publicar.

Migrações hospedadas rodam no início do build da Vercel e somente com `APP_ENV` e banco próprios do projeto. A ferramenta usa trava transacional, registra checksum e impede alterar uma migração aplicada; abrir uma página nunca executa migração. O cache do build inclui scripts e arquivos de migração. Para mudanças futuras, adicionar outra migração e preferir expansão compatível antes de remoção. Antes de migração em produção, criar backup/restauração no provedor e verificar recuperação. Rollback de código não desfaz alterações no banco.
